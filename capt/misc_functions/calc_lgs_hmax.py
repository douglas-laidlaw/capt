...
def calc_hmax(pupil_mask, n_subap, gs_pos, gs_dist, tel_diam, air_mass):
	...
	for i in range(combs):
		...
		gs_dist = numpy.mean(numpy.array([gs_dist0, gs_dist1]))
		...
		if gs_dist>0:
			maxObsAlt = (gs_dist*(real_tel_diam-w))/(sep*gs_dist + (real_tel_diam-w))
			maxObsAlt /= air_mass
