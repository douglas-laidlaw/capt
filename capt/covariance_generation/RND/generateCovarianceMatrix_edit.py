...
from superFastCovMap import matrixSubapSep, getMappingMatrix, superFastCovMap
...
class CovarianceMatrix(object):
    ...
    Parameters:
        n_wfs (int): Number of wavefront sensors present.
         (ndarray): A map of the pupil - is nx_subaps by ny_subaps. 1 if subap active, 0 if not.
    ...
    def __init__(
        self, n_wfs, pupil_mask, telescope_diameter, subap_diameters, gs_altitudes, gs_positions, wfs_wavelengths,
        n_layers, layer_altitudes, offsets, fitL0, L0, pupilShift, pupilRot, stycMethod, threads=1):
        ...
        self.n_subaps = numpy.array([pupil_mask.sum()]*self.n_wfs).astype('int')
        self.total_subaps = self.n_subaps.sum()
        ...
        self.translation = numpy.zeros((self.n_layers, self.n_wfs, 2))
        self.subap_layer_diameters = numpy.zeros((self.n_layers, self.n_wfs)).astype("float64")
        self.subap_parameters()

        if self.offsets==False:
            self.mm, mmc, md = getMappingMatrix(self.pupil_mask, numpy.ones((self.n_subaps[0], self.n_subaps[0])))
            mapOnes = superFastCovMap((pupil_mask.shape[0]*2)-1, numpy.ones((self.n_subaps[0], self.n_subaps[0])), self.mm, mmc, md)
            ...
            yMapSep = mapOnes.copy() * numpy.arange(-(pupil_mask.shape[0]-1), (pupil_mask.shape[0])) * self.telescope_diameter/pupil_mask.shape[0]
            yMapSepFlat = yMapSep.flatten()
            xMapSep = yMapSep.copy().T
            xMapSepFlat = xMapSep.flatten()
            
            self.xy_separations = numpy.array((xMapSepFlat[self.flatMM==1], yMapSepFlat[self.flatMM==1])).T
            self.xy_separations += 1e-20

        else:
            # self.xy_separations = False
            ...
            #     wfs_subap_pos = (numpy.array(numpy.where(self.pupil_mask== 1)).T * self.telescope_diameter/self.pupil_mask.shape[0])
            #     self.subap_positions[wfs_n] = wfs_subap_pos

            wfs_subap_pos = (numpy.array(numpy.where(self.pupil_mask== 1)).T * self.telescope_diameter/self.pupil_mask.shape[0])
            self.subap_positions = numpy.array([wfs_subap_pos]*self.n_wfs)
            # print self.subap_positions.shape
            # print self.subap_positions
            self.subap_positions = self.subapPos_wfsAlignment(pupilShift, pupilRot)
            ...
            # self.misalignedParameters()
        
        if self.fitL0==False and self.offsets==False:
            self.profileParameters(L0)



    def make_covariance_matrix(self, r0, L0, pupilShift, pupilRot):
        if self.fitL0 == True or self.offsets==True:
            if self.offsets==True:
                # self.subap_positions = self.subapPos_wfsAlignment(pupilShift, pupilRot)
                self.profileParameters(L0)
            else:
                self.profileParameters(L0)
    ...
    def uniqueMappingMatrix(self, mm):
        uniqueMM = numpy.zeros(mm.shape)
        lineRef = numpy.zeros(mm.shape[1])
        for i in range(mm.shape[0]):
            ...
            if lineRef.max()==1:
                ...
                if lineRef.max()==2:
                    uniqueMM[i, lineRef!=2] += mm[i, lineRef!=2]
            lineRef[lineRef>1] /= lineRef[lineRef>1]

        return uniqueMM.astype('int'), lineRef.astype('int')


    def subap_parameters(self):
        """
        Calculate initial parameters that are fixed under zero offset.
        """
    ...
    def subapPos_wfsAlignment(self, pupilShift, pupilRot):
        ...
        for layer_n in range(self.n_layers):
            ...
            for wfs_i in range(self.n_wfs):
                ...
                for wfs_j in range(wfs_i+1):
                    ...
                    #Only first auto-covariance and all WFS combinations generated
                    if wfs_i!=wfs_j or (wfs_i==0 and wfs_j==0):
                                            
                        if self.offsets==True:
                            cov_xx, cov_yy, cov_xy = wfs_covariance(
                                    self.n_subaps[wfs_i], self.n_subaps[wfs_j],
                                    self.subap_positions[wfs_i], self.subap_positions[wfs_j],
                                    self.subap_layer_diameters[layer_n, wfs_i], self.subap_layer_diameters[layer_n, wfs_j],
                                    L0[layer_n], False, self.offsets, self.translation[layer_n, wfs_i], 
                                    self.translation[layer_n, wfs_j], self.stycMethod)
                        else:
                            cov_xx, cov_yy, cov_xy = wfs_covariance(
                                    self.n_subaps[wfs_i], self.n_subaps[wfs_j],
                                    False, False,
                                    self.subap_layer_diameters[layer_n, wfs_i], self.subap_layer_diameters[layer_n, wfs_j],
                                    L0[layer_n], self.xy_separations, self.offsets, self.translation[layer_n, wfs_i], 
                                    self.translation[layer_n, wfs_j], self.stycMethod)
                            # print self.subap_layer_separations[layer_n, wfs_j], '\n'

                            self.fill_xx[self.flatMM==1] = cov_xx
                            cov_xx = (self.mm * self.fill_xx)[self.mm==1].reshape(self.n_subaps[wfs_i], self.n_subaps[wfs_j])

                            self.fill_yy[self.flatMM==1] = cov_yy
                            cov_yy = (self.mm * self.fill_yy)[self.mm==1].reshape(self.n_subaps[wfs_i], self.n_subaps[wfs_j])

                            self.fill_xy[self.flatMM==1] = cov_xy
                            cov_xy = (self.mm * self.fill_xy)[self.mm==1].reshape(self.n_subaps[wfs_i], self.n_subaps[wfs_j])

                    #Set up repetition of first auto-covariance
                    if wfs_i==0 and wfs_j==0:
                        autoCov_xx = cov_xx
                        autoCov_yy = cov_yy
                        autoCov_xy = cov_xy
                    if wfs_i==wfs_j:
...
def wfs_covariance(n_subaps1, n_subaps2, wfs1_positions, wfs2_positions, 
                    wfs1_diam, wfs2_diam, L0, xy_seps, offsets, wfs1_trans, wfs2_trans, stycMethod):
    """
    ...
    diag1 = numpy.arange(1,35)
    diag2 = numpy.arange(0,34)

    if offsets==True:
        xy_separations = calculate_wfs_separations(n_subaps1, n_subaps2, wfs1_positions, wfs2_positions) - (wfs2_trans - wfs1_trans)
        cov_xy_correction = 1.
        # print xy_separations[diag2, diag1]
    else:
        xy_separations = xy_seps + (wfs2_trans - wfs1_trans)
        cov_xy_correction = -1.

    if stycMethod == True:
        # print("Min separation: {}".format(abs(xy_separations).min()))
        cov_xx = compute_covariance_xx(xy_separations, wfs1_diam*0.5, wfs2_diam*0.5, L0)
        cov_yy = compute_covariance_yy(xy_separations, wfs1_diam*0.5, wfs2_diam*0.5, L0)
        cov_xy = compute_covariance_xy(xy_separations, wfs1_diam*0.5, wfs2_diam*0.5, L0)
        
        return cov_xx, cov_yy, cov_xy * cov_xy_correction
...
def compute_covariance_xx(separation, subap1_rad, subap2_rad, L0):
    ...
    diag1 = numpy.arange(0,35)
    diag2 = numpy.arange(1,36)


    return Cxx


def compute_covariance_yy(separation, subap1_rad, subap2_rad, L0):
...
def compute_covariance_xy(seperation, subap1_rad, subap2_rad, L0):
...
if __name__ == "__main__":
    ...
    # GSPOS = numpy.array([[0.,0.], [0.,0], [0,0]])#, [50,50], [-5, -5]])
    ...
    params = CovarianceMatrix(NWFS, PUPIL_MASK, TEL_DIAM, SUBAPDIAM, GSALT, GSPOS, gam, 
        NLAYERS, LAYERHEIGHTS, offsets, fitL0, L0, PUPILSHIFT, PUPILROT, True)
    s = time.time()
    ...
    belowGround = 6
    envelope = 6
    ones = numpy.ones((int(PUPIL_MASK.sum()), int(PUPIL_MASK.sum())))
    mm, mmc, md = getMappingMatrix(PUPIL_MASK, ones)

    m = covMapsFromMatrix(nmat, GSPOS, NXSUBAPS[0], NSUBAPS[0], PUPIL_MASK, output, mm, mmc, md)
    l = covSlicesFromMaps(m, GSPOS, PUPIL_MASK, selector, belowGround, envelope)
