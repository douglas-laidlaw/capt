...
def matcov(
	...
	print '\n', 'BREAK', '\n'
	print subapLayerPos[0]
	print subapLayerPos[1]
				...
				for j in range(joff, Nj):
					...
					for l in range(0, nLayers):
						...
						if subapSizes[m, l]>0 and subapSizes[n,l]>0:
							...
							tdu = (subapLayerPos[0, 0, 35, l]
									- subapLayerPos[0, 1, 35, l])

							tdv = (subapLayerPos[1, 0, 35, l]
									- subapLayerPos[1,1, 35, l])
...
if __name__=='__main__':
	...
	radSqaured_to_arcsecSqaured = ((180./numpy.pi) * 3600)**2
	NLAYERS = 1
	...
	o = matcov(NWFS, PUPIL_MASK, TEL_DIAM, NSUBAPS, NXSUBAPS, SUBAPDIAM, GSALT, GSPOS, NLAYERS, LAYERHEIGHTS, r0, L0, covMat, PUPILMAG, wfsRot=PUPILROT) * radSqaured_to_arcsecSqaured
	
	ones = numpy.ones((NSUBAPS[0], NSUBAPS[0]))
	mm, mmc, n = getMappingMatrix(PUPIL_MASK, ones)
	om = covMapsFromMatrix(o, GSPOS, NXSUBAPS[0], NSUBAPS[0], PUPIL_MASK, 'x and y', mm, mmc, n)
