...
class covariance_roi(object):
	...
	def __init__(self, pupil_mask, subap_diam, wavelength, tel_diam, n_subap, gs_alt, gs_pos, allMapPos, 
		xy_separations, n_layer, layer_alt, wind_profiling, tt_track_present, lgs_track_present, 
		offset_present, fit_layer_alt, fit_tt_track, fit_lgs_track, fit_offset, fit_L0, L0, roi_axis, styc_method):
	...
	def fixedLayerParameters(self):
		...
		if self.roi_axis=='x' or self.roi_axis=='y' or self.roi_axis=='x+y':
			self.covariance_slice_fixed = numpy.zeros((self.n_layer, self.combs*self.roi_width, self.roi_length))
			# if self.tt_track_present==True or self.fit_tt_track==True:
			# 	self.track_locations = numpy.zeros((self.combs*self.roi_width, self.roi_length))
		
		if self.roi_axis=='x and y':
			self.covariance_slice_fixed = numpy.zeros((self.n_layer, self.combs*self.roi_width, self.roi_length*2))
			# if self.tt_track_present==True or self.fit_tt_track==True:
			# 	self.track_locations = numpy.zeros((self.combs*self.roi_width, self.roi_length*2))
...
if __name__ == "__main__":
	...
	params = covariance_roi(pupil_mask, subap_diam, wavelength, tel_diam, n_subap, gs_alt, gs_pos, allMapPos, xy_separations, 
		n_layer, layer_alt, False, tt_track_present, lgs_track_present, offset_present, False, fit_tt_track, fit_lgs_track, 
		fit_offset, False, L0, roi_axis, styc_method)
	s = time.time()
	t_cov_roi = params._make_covariance_roi_(layer_alt, r0, L0, tt_track, lgs_track, shwfs_shift, shwfs_rot, delta_xSep=delta_xSep, delta_ySep=delta_ySep)
	f = time.time()
	print('Time Taken: {}'.format(f-s))

	pyplot.figure()
	pyplot.imshow(t_cov_roi)