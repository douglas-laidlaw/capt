...
class covarianceSlice(object):
    ...
    def __init__(self, nwfs, subapDiam, wfsWavelength, telDiam, gsAlt, gsPos, subapLoc, selector, nLayers, layerAlt, fitOffset, 
                fitRot, fitL0, offset, rot, L0,  output, styc):
        ...
        self.output = output
        self.styc = styc
        rot = rot * numpy.pi/180.

        sr = self.subapLoc.copy()
            ...
            for wfs_n in range(2):
                for comb in range(self.combs):                        
                    ...
                    if self.gsAlt[self.selector[comb, wfs_n]] != 0:
                        ...
                        positions = sr[wfs_n][comb]
                        
                    xtp = sr[wfs_n][comb].T[0]
                    ytp = sr[wfs_n][comb].T[1]

                    th = rot[self.selector[comb, wfs_n]]
        ...
        print self.subap_layer_positions[:,0]
        print self.subap_layer_positions[:,1]
        print '\n', 'BREAK', '\n'

        sr = self.subapLoc.copy()
        # print sr[1,0,4] - self.subap_layer_positions[:,1,0,4]
        # print '\n', 'BREAK', '\n'

        self.xy_separations = self.subap_layer_positions[:,0,:,:] - self.subap_layer_positions[:,1,:,:]
        print self.xy_separations
        # self.xy_separations[0,0,0,0] = numpy.array([0.682842712475, 0.682842712475]) 
        print self.xy_separations
        # self.xy_separations = self.subap_layer_positions[:,0,:,:,:,0] - self.subap_layer_positions[:,1,:,:,:,1]
        ...
        for layer_n in range(self.nLayers):
            ...
            for comb in range(self.combs):
                ...
                for env in range(self.envelope):
                    ...
                    # cov_xx, cov_yy = wfs_covariance(self.nSubaps[self.selector[comb][0]], self.subap_layer_diameters[layer_n, 0, comb, env], 
                    #         self.xy_separations[layer_n, comb, env], self.flatSF[count: count+self.sfFinder[layer_n, comb, env]] , self.styc)
                    ...
                    # print '\n', self.flatSF[count: count+self.sfFinder[layer_n, comb, env]].shape
                    count += self.sfFinder[layer_n, comb, env]
                    # print count
...
if __name__ == '__main__':
    ...
    params = covarianceSlice(nwfs, subapDiam, wfsWavelength, telDiam, gsAlt, gsPos, subapLoc, selector, nLayers, layerAlt, fitOffset, 
                fitRot, fitL0, offset, rot, L0, output, styc)
