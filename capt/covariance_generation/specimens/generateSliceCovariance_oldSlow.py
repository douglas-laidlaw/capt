...
# from aotools import slopecovariance
from covMapsFromMatrix import covMapsFromMatrix

 
class covarianceSlice(object):
     
    def __init__(self, nwfs, subapDiam, wfsWavelength, telDiam, gsAlt, gsPos, subapLoc, selector, nLayers, layerAlt, output, styc):
    ...
    def getCovarianceSlice(self, r0, L0):
        ...
        self._make_covariance_slice(r0, L0)

        return self.covariance_slice
        ...
        for layer_n in range(self.nLayers):
            ...
            for comb in range(self.combs):
                ...
                for env in range(self.envelope):
                    ...
                    cov_xx, cov_yy = wfs_covariance(
                            self.nSubaps[self.selector[comb][0]], self.nSubaps[self.selector[comb][1]],
                            self.subap_layer_positions[layer_n, 0, comb, env], self.subap_layer_positions[layer_n, 1, comb, env],
                            self.subap_layer_diameters[layer_n, 0, comb, env], self.subap_layer_diameters[layer_n, 1, comb, env],
                            r0[layer_n], L0[layer_n], self.styc)

                    # print("covmat coords: ({}: {}, {}: {})".format(cov_mat_coord_x1, cov_mat_coord_x2, cov_mat_coord_y1, cov_mat_coord_y2))
                    r0_scale = ((self.wfsWavelength[self.selector[comb,0]] * self.wfsWavelength[self.selector[comb,1]])
                            / (8 * numpy.pi**2 * self.subap_layer_diameters[layer_n, 0, comb, env] * self.subap_layer_diameters[layer_n, 0, comb, env]))

                    if self.output=='x+y':
                        self.covariance_slice[self.envelope*comb + env] += ((cov_xx+cov_yy)*(r0_scale/2.))

                    if self.output=='x':
                        self.covariance_slice[self.envelope*comb + env] += cov_xx*r0_scale

                    if self.output=='y':
                        self.covariance_slice[self.envelope*comb + env] += cov_yy*r0_scale

                    if self.output=='x and y':
                        self.covariance_slice[self.envelope*comb + env] += numpy.hstack(((cov_xx*r0_scale), (cov_yy*r0_scale)))


def wfs_covariance(n_subaps1, n_subaps2, wfs1_positions, wfs2_positions, wfs1_diam, wfs2_diam, r0, L0, styc):
    """
    ...
    Parameters:
        ...
        mapCoords: XY sub-aperture separation coordinates along covariance map vector - determined by wavefront angle of arrival. 
        stycMethod: If True use phase structure function differencing method, otherwise use numerical integration method
        xy_covariaqnce: Whether to included xy covariance - 'True' or 'False'
        covMapSlice: Whether to only calculate covariance along wavefront angle of arrival - 'True' or 'False' 

    Returns:
        slope covariance of X with X , slope covariance of Y with Y, slope covariance of X with Y
    """        
    xy_separations = wfs2_positions - wfs1_positions
    xy_separations += 1e-20

    if styc == True:
        # print("Min separation: {}".format(abs(xy_separations).min()))
        cov_xx = compute_covariance_xx(xy_separations, wfs1_diam, wfs2_diam, r0, L0)
        cov_yy = compute_covariance_yy(xy_separations, wfs1_diam, wfs2_diam, r0, L0)
        ...
        maxDelta=0
        for i in range(xy_separations.shape[0]):
            if math.isnan(xy_separations[i,0]) != True:
                if numpy.abs(xy_separations[i]).max()>maxDelta:
                    maxDelta = int(numpy.abs(xy_separations[i]).max()/wfs1_diam)+1
        
        sf_dx = wfs1_diam/100.
        sf_n = int(4*maxDelta*(wfs1_diam/sf_dx))
        # print numpy.arange(sf_n)*sf_dx, (numpy.arange(sf_n)*sf_dx).shape
        # f=p
        sf = new_structure_function_vk(numpy.arange(sf_n)*sf_dx, r0, L0)
        sf[0] = 0.    #get rid of nan

        nSamp = 8    #hard-wired parameter
        cov = compute_ztilt_covariances(n_subaps1, xy_separations, sf, sf_dx, nSamp, wfs1_diam)
...
def compute_covariance_xx(separation, subap1_diam, subap2_diam, r0, L0):    
    cov_xx = numpy.zeros((separation.shape[0]),numpy.float)
    
    for n in range(cov_xx.shape[0]):
        if math.isnan(separation[n,0]) != True:

            x1 = separation[n,1] + (subap2_diam - subap1_diam) * 0.5
            r1 = numpy.array([numpy.sqrt(x1**2 + separation[n,0]**2)])

            x2 = separation[n,1] - (subap2_diam + subap1_diam) * 0.5
            r2 = numpy.array([numpy.sqrt(x2**2 + separation[n,0]**2)])

            x3 = separation[n,1] + (subap2_diam + subap1_diam) * 0.5
            r3 = numpy.array([numpy.sqrt(x3**2 + separation[n,0]**2)])

            cov_xx[n] = (-2 * new_structure_function_vk(r1, r0, L0)
                    + new_structure_function_vk(r2, r0, L0)
                    + new_structure_function_vk(r3, r0, L0))
...
def compute_covariance_yy(separation, subap1_diam, subap2_diam, r0, L0):
    cov_yy = numpy.zeros((separation.shape[0]),numpy.float)
    
    for n in range(cov_yy.shape[0]):
        if math.isnan(separation[n,0]) != True:

            y1 = separation[n,0] + (subap2_diam - subap1_diam) * 0.5
            r1 = numpy.array([numpy.sqrt(separation[n,1]**2 + y1**2)])

            y2 = separation[n,0] - (subap2_diam + subap1_diam) * 0.5
            r2 = numpy.array([numpy.sqrt(separation[n,1]**2 + y2**2)])

            y3 = separation[n,0] + (subap2_diam + subap1_diam) * 0.5
            r3 = numpy.array([numpy.sqrt(separation[n,1]**2 + y3**2)])

            cov_yy[n] = (-2 * new_structure_function_vk(r1, r0, L0)
                + new_structure_function_vk(r2, r0, L0)
                + new_structure_function_vk(r3, r0, L0))
...
# def structure_function_vk(separation, r0, L0):
#     """
...
#                 * (1 - 2 * numpy.pi ** (5. / 6.) * ((separation) / L0) ** (5. / 6.)
#                 / scipy.special.gamma(5. / 6.)
#                 * scipy.special.kv(5. / 6., (2 * numpy.pi * separation) / L0))
#             )
...
def new_structure_function_vk(separation, r0, L0):
...
if __name__ == '__main__':
    ...
    # pupilMasks = pupil.circle(nxSubaps[0]/2., nxSubaps[0])
    # pupilMasks[3,3]=0
    ...
    gParams = covarianceSlice(nwfs, subapDiam, wfsWavelength, telDiam, gsAlt, gsPos, subapLoc, selector, nLayers, layerAlt, output, stycMethod)
    s = gParams.getCovarianceSlice(r0, L0)